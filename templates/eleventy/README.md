# Serverless Cloud Eleventy Project

This is a scaffolding project for building an Eleventy static site with an API on Serverless Cloud!
...
To make changes to your Eleventy site, open a separate terminal and run `npm i`. Once the dependencies are installed, run `cloud dev`. You can then make changes and view them locally. Eleventy files are in the `_site` directory. A local proxy is running that allows you to access your live developer sandbox using `/api` from your local Eleventy dev server. When you are ready to deploy your changes to your developer sandbox, type `build` in the cloud shell, or `cloud build` in a terminal window.

To create a preview version to share with others, type `share` in the cloud shell, or `cloud share` in the terminal window. And to deploy to a stage, type `deploy` in the cloud shell, or `cloud deploy` in your terminal.

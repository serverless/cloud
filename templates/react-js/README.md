# Serverless Cloud React.js Project with API

This is a scaffolding project for building a React.js project with an API on Serverless Cloud!
...
To make changes to React, open a separate terminal and run `npm i`. Once the dependencies are installed, run `cloud dev`. You can then make changes and view them locally. A local proxy is running that allows you to access your live developer sandbox using `/api` from your local React.js dev server. When you are ready to deploy your React changes to your developer sandbox, type `build` in the cloud shell, or `cloud build` in a terminal window.
